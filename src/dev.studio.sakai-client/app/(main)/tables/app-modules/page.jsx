...
export default function Modules() {
    ...
    const backendServiceUrl = `${process.env.app_protocol}://${process.env.app_host}:${process.env.app_port}/AppModule`
    ...
    const [detailSchemaName, setDetailSchemaName] = useState('')
    const [detailOltpModuleName, setDetailOltpModuleName] = useState('')
    ...
    useEffect(() => {
        async function fetchItems() {
            ...
            if(items !== null && items.length > 0 && selectedRowIndex < items.length) {
                ...
                setDetailSchemaName(items[selectedRowIndex].schemaName)
                setDetailOltpModuleName(items[selectedRowIndex].oltpModuleName)

                console.log('end of setting selectedRow')
            }
            else {

                console.log('set init selected row state to empty')
                ...
                setDetailName('')
                setDetailSchemaName('')
                setDetailOltpModuleName('')
            }
        }

        console.log("before setItemList")
        fetchItems()
...
return (
    <div>
        <h1>AppModules</h1>
        <p>This is the AppModules page.</p>
                    ...
                    <DataTable 
                        ...
                        onSelectionChange={(e) => {
                            if(e.value !== null){
                                ...
                                setDetailSchemaName(e.value.schemaName)
                                setDetailOltpModuleName(e.value.oltpModuleName)
                            }
                        }
                        }
                        dataKey="id"
                ...
                <div className="flex card flex-column gap-4">
                    <FloatLabel>
                        <InputText readOnly={true}
                            ...
                            value={detailId}
                        ></InputText>
                        <label htmlFor="moduleId">Id</label>
                    </FloatLabel>
                    <FloatLabel>
                        <InputText 
                            id="moduleCode"
                    ...
                    <FloatLabel>
                        <InputText 
                            id="schemaName"
                            value={detailSchemaName}
                            onChange={(e) => {
                                setDetailSchemaName(e.target.value)
                                setIsFormUpdated(true);
                            }
                        }
                        ></InputText>
                    ...
                    <FloatLabel>
                        <InputText 
                            id="oltpModuleName"
                            value={detailOltpModuleName}
                            onChange={(e) => {
                                setDetailOltpModuleName(e.target.value)
                                setIsFormUpdated(true);
                            }
                        }
                        ></InputText>
                        <label htmlFor="oltpModuleName">OLTP Module Name</label>
                    </FloatLabel>
                        ...
                        if(isFormUpdated) {
                            ...
                            const updatedItemResponse = await fetch(queryUrl, {
                                ...
                                body: JSON.stringify({
                                    ...
                                    OltpModuleName: detailOltpModuleName
                                })
                            })
                    ...
                    {isFormUpdated &&
                        <Button label="Cancel" onClick={() => {
                            ...
                            setDetailSchemaName(selectedRow.SchemaName)
                            setDetailOltpModuleName(selectedRow.OltpModuleName)
                            setIsFormUpdated(false)
                        }}></Button>
                    }
